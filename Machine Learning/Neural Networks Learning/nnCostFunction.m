...
for i = 1:m,
  ny(i, y(i)) = 1;
  tot = tot + sum(ny(i, :) .* (log(h(i, :)))) + sum((one - ny(i, :)) .* log(one - (h(i, :))));
endfor
...
reg = 0;
sqtheta1 = zeros(hidden_layer_size, input_layer_size);
sqtheta1 = Theta1(:, 2:end) .^ 2;
sqtheta2 = zeros(num_labels, hidden_layer_size);
sqtheta2 = Theta2(:, 2:end) .^ 2;
reg = sum(sum(sqtheta1, 1)) + sum(sum(sqtheta2, 1));
